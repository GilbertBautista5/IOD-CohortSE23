...
// 3. add "type": "module", to package.json (just before the dependencies)
...
fetch('https://jsonplaceholder.typicode.com/posts/10') //sample public JSON API for use in testing
    ...
    .then(json => console.log(`Post #${json.id}: ${json.title}`)) //this promise includes the json returned from the fetched URL if successful
...
let post2Json = await post2.json() //just grab the json from the response - also via an awaited promise
...
let post5Title = await fetchPostTitle(5)
console.log('Post #5 title: '+post5Title)
...
fetchPostTitle(3).then(title => console.log(`Post #3 Title: ${title}`))